...
# 1980 men
occ_1980_w_m <- data_clean %>%
  filter(YEAR==1980) %>%
  filter(SEX==1) %>%
  filter(RACE==1) %>%
  select(OCC_2DIGIT)

occ_1980_b_m <- data_clean %>%
  filter(YEAR==1980) %>%
  filter(SEX==1) %>%
  filter(RACE==2) %>%
  select(OCC_2DIGIT)
  
table_1980_w_m <- table(occ_1980_w_m$OCC_2DIGIT) %>% 
  as.data.frame() %>%
  rename(OCC_2DIGIT = Var1, W_M_1980 = Freq)
table_1980_w_m

table_1980_b_m <- table(occ_1980_b_m$OCC_2DIGIT) %>% 
  as.data.frame() %>%
  rename(OCC_2DIGIT = Var1, B_M_1980 = Freq)
table_1980_b_m

index_1980_m <- full_join(table_1980_w_m, table_1980_b_m)
index_1980_m

dissim(data = index_1980_m[,2:3])
```
...
occ_1980_w_m <- data_clean %>%
  ...
  filter(RACE==1) %>%
  select(OCC_2DIGIT) %>%
  rename(OCC_2DIGIT_WM1980 = OCC_2DIGIT)

# doesn't work
index_1980_men <- data_clean %>%
  filter(YEAR==1980) %>%
  filter(SEX==1) %>%
  mutate(INCWAGE_ADJ_BLACK = INCWAGE_ADJ[data_clean$RACE==2]) %>%
  mutate(INCWAGE_ADJ_WHITE = INCWAGE_ADJ[data_clean$RACE==1]) %>%
  dissim()
...
index_1980_men <- data_clean %>%
  filter(YEAR==1980) %>%
  filter(SEX==1) %>%
  select(RACE, INCWAGE_ADJ) %>%
  dissim()


######## belicia's code for index of dissim

#male 1980 
#data_clean %>% 
#  filter(YEAR == 1980, SEX ==1) %>% 
#  count(RACE == 1)

#male_1980 <- data_clean %>% 
#  mutate(RACE_NEW = ifelse(RACE == 1, 
#                       "white", 
#                       "black"))%>% 
#  filter(YEAR == 1980, SEX == 1) %>% 
#  group_by(OCC1990, RACE_NEW)%>% 
#  summarise(count = length(RACE_NEW)) %>% 
#  pivot_wider(names_from = RACE_NEW, 
#              values_from = count) %>% 
#  mutate(white_diff = white/1597437) %>% 
#  mutate(black_diff = black/168400) %>% 
#  mutate (d = white_diff - black_diff) %>% 
#  mutate (d = abs(d))

# male1980 <- 0.5*sum(male_1980$d)
```
