...
# test = group by year, then filter out top and bottom 1%
test <- data_clean %>%
  group_by(YEAR) %>%
  filter(quantile(INCWAGE_ADJ, 0.99) > INCWAGE_ADJ) %>%
  filter(quantile(INCWAGE_ADJ, 0.01) < INCWAGE_ADJ)

# test 2 = filter out top and bottom percentiles across all years (no grouping first) 
test2 <- data_clean %>%
  filter(INCWAGE_ADJ > 600) %>%
  filter(INCWAGE_ADJ < 160000)

quantile(test$INCWAGE_ADJ, c(.01, .99))
quantile(test2$INCWAGE_ADJ, c(.01, .99))

#### test

tapply(data_clean$INCWAGE_ADJ, data_clean$YEAR, quantile, probs=c(0.01, 0.99))

max(test$INCWAGE_ADJ[test$YEAR==1980])
max(test2$INCWAGE_ADJ[test2$YEAR==1980])

max(test$INCWAGE_ADJ[test$YEAR==1990])
max(test2$INCWAGE_ADJ[test2$YEAR==1990])

max(test$INCWAGE_ADJ[test$YEAR==2000])
max(test2$INCWAGE_ADJ[test2$YEAR==2000])

max(test$INCWAGE_ADJ[test$YEAR==2010])
max(test2$INCWAGE_ADJ[test2$YEAR==2010])
...
min(test$INCWAGE_ADJ[test$YEAR==1980])
min(test2$INCWAGE_ADJ[test2$YEAR==1980])

min(test$INCWAGE_ADJ[test$YEAR==1990])
min(test2$INCWAGE_ADJ[test2$YEAR==1990])

min(test$INCWAGE_ADJ[test$YEAR==2000])
min(test2$INCWAGE_ADJ[test2$YEAR==2000])

min(test$INCWAGE_ADJ[test$YEAR==2010])
min(test2$INCWAGE_ADJ[test2$YEAR==2010])

#### I THINK THE ABOVE IS RIGHT TO FILTER OUT TOP AND BOTTOM 1% BY YEAR?!?! CHECK AGAIN
...
data_clean %>% filter(YEAR==1980) %>%
  quantile(INCWAGE_ADJ, c(.01, .99))
...
#### need to order these categories?? no b/c they are binary, right?
